...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  // Use local state for session messages
  const [sessionMessages, setSessionMessages] = useState<ChatMessage[]>([{
    id: 'welcome-message',
    role: 'assistant',
    text: "Just type or say something like 'Spent 500 on lunch'. Make sure you are saving your budget value before logging an expense.",
    timestamp: new Date() 
  }]);
      ...
      if (parsed && typeof parsed.amount === 'number') {
        ...
        if (result.error) {
          ...
          const successMsg: ChatMessage = {
            id: (Date.now() + 1).toString(),
            role: 'assistant',
            text: `Got it! ₹${result.expense.amount.toLocaleString('en-IN')} for "${result.expense.description}" added. ₹${result.budgetLeft.toLocaleString('en-IN')} left in ${result.expense.category}.`,
            timestamp: new Date(),
            metadata: { addedExpense: result.expense, budgetLeft: result.budgetLeft }
          };
          setSessionMessages(prev => [...prev, successMsg]);
        }
      } else {
        ...
        setSessionMessages(prev => [...prev, hintMsg]);
      }
    } catch (err) {
      console.error("Chat process error:", err);
      const techError: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        text: "I ran into a technical issue. Please try again later.",
        timestamp: new Date(),
      };
      setSessionMessages(prev => [...prev, techError]);
    }
    setIsTyping(false);
  };
