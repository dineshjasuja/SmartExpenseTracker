...
const App: React.FC = () => {
  ...
  const handleUpdateBudget = async (category: string, limit: number) => {
    setState(prev => {
      ...
      if (existingIndex > -1) {
        return {
          ...prev,
          budgets: prev.budgets.map((b, i) => i === existingIndex ? { ...b, limit } : b)
        };
      }
      return prev;
    });
  ...
  const handleSaveAllBudgets = async () => {
    setIsSyncing(true);
    try {
      await apiService.saveBudgets(state.budgets);
    } catch (e) {
      console.error("Failed to save budgets on server", e);
      throw e;
    } finally {
      setIsSyncing(false);
    }
  ...
  if (isLoading) {
    return (
      <div className="w-full h-[100dvh] bg-indigo-600 flex flex-col items-center justify-center text-white">
        ...
        <p className="text-indigo-100 opacity-60 text-sm mt-2 font-bold uppercase tracking-widest">Securing your session</p>
      </div>
    );
  }
      ...
      <main className="flex-1 overflow-hidden relative bg-gray-50/30">
        ...
        {activeTab === ActiveTab.SETTINGS && (
          <Settings 
            ...
            onSaveAllBudgets={handleSaveAllBudgets}
